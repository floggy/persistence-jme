...
* <br>
* <code> MigrationManager manager = MigrationManager.getInstance();<br>
* *  String[] notMigratedClasses = manager.getNotMigratedClasses();<br>
* * <br>
* *  for (int i = 0; i &lt; notMigratedClasses.length; i++) {<br>
* * <br>
...
* manager.start(Class.forName(notMigratedClasses[i]), null);<br>
...
public abstract class MigrationManager {
	/**
	* It enables the developer to iterate over all registers without update
	* or delete it.
	 */
	public static final String ITERATION_MODE = "ITERATION_MODE";

	/**
	* It drives the migration avoiding or not the loading of Persistable
	* fields
	 */
	...
	*/
	public static MigrationManager getInstance() {
		if (instance == null) {
			try {
				Class pmClass =
					Class.forName(
						"net.sourceforge.floggy.persistence.impl.migration.MigrationManagerImpl");
	...
	/**
	* Finish the whole migration process. At the end it update the layout
	* of fields of the Persistable class being migrated.
	*
	* @param persistableClass The Persistable being migrated.
	*
	* @throws FloggyException If the enumeration wasn't whole processed.
	*/
	...
	* @param persistableClass The Persistable being migrated.
	*
	* @throws FloggyException A exception that holds the underling problem.
	*/
	...
	*
	* @param persistableClass The Persistable being migrated.
	* @param properties A list of properties that will driven the migration.
	*
	* @return A enumeration
	*
	* @throws FloggyException A exception that holds the underling problem.
	*/
	public abstract Enumeration start(Class persistableClass, Hashtable properties)
		throws FloggyException;
}
