...
public class FlowManager {
	...
	public static FlowManager getInstance()
			throws FlowManagerNotStartedException {
		if (instance == null)
	...
	public void exibirDadosChurrasForm() {
		...
		barbecueForm.append(new TextField("# men", "", 3,
				TextField.NUMERIC));
		barbecueForm.append(new TextField("# woman", "", 3,
				TextField.NUMERIC));
	...
	public void calculateBarbecue() {
		...
		boolean withCaipirinha = ((ChoiceGroup) barbecueForm.get(2))
				.isSelected(0);
		...
		if (men == 0 && women == 0) {
			...
			try {
				DisplayManager.getInstance().show(
						new Alert("Warn!",
								"Every barbecue has to have drink and food!!!",
								null, AlertType.ERROR));
			} catch (DisplayManagerException e) {
			}
		} else {
			barbecue = new Barbecue(men, women, alchoolLevel,
					foodQuantity, withCaipirinha);
	...
	public void salvarChurrasco() {
		...
		salvarChurrasForm.append(new TextField(
				"Give a name to the barbecue to save: ", "", 30,
				TextField.ANY));
	...
	public void finalizarPersistenciaChurras() {
		...
		try {
			...
			DisplayManager.getInstance().show(
					new Alert("Success", "Barbecue saved!", null,
	...
	public void exibirListaChurrasSalvos() {
		List churrasSalvosList = new List("Saved barbecues", List.EXCLUSIVE
				| List.IMPLICIT);
		churrasSalvosList.setFitPolicy(Choice.TEXT_WRAP_OFF);

		try {
			Barbecue temp = new Barbecue();
			ObjectSet os = PersistableManager.getInstance().find(
					temp.getClass(), null, null);
			int size = os.size();
		...
		if (savedBarbecues.length == 0) {
			try {
				DisplayManager.getInstance().show(
						new Alert("Warn", "There isn't barbecues saved.", null,
								AlertType.INFO));
			} catch (DisplayManagerException e) {
			}
		} else {
			for (int i = 0; i < savedBarbecues.length; i++)
				churrasSalvosList
						.append(savedBarbecues[i].getDescription(), null);
