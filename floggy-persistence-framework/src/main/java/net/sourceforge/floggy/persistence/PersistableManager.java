...
public abstract class PersistableManager {
	...
	public static PersistableManager getInstance() {
		if (instance == null) {
			try {
				Class pmClass =
					Class.forName(
						"net.sourceforge.floggy.persistence.impl.PersistableManagerImpl");
				instance = (PersistableManager) pmClass.newInstance();
			} catch (ClassNotFoundException cnfex) {
				throw new RuntimeException(
					"No PersistableManager implementation was found. Please check the weaver execution.");
	...
	* The main use of this method is to import data  from a remote source.<br>
	* Set the property BATCH_MODE to true to improve  the performance and
	* create only one object to save the data. <br>
	* <br>
	* <code> PersistableManager manager = PersistableManager.getInstance();<br>
	...
	* It uses an index as the base mechanism to evaluate  the registers in the
	* repository<br>
	*
	* @param persistableClass The persistable class to search the objects.
	...
	*/
	...
	/**
	* Searches objects of a specific persistable class from the repository. <br>
	* <br>
	* An optional application-defined search criteria can be  defined using a <code>Filter</code>.<br>
	* <br>
	* An optional application-defined sort order can be defined using a
